...
- Area docs follow the root pattern: content in `<area>/AGENTS.md`, loaded through its one-line `<area>/CLAUDE.md` shim. Today only `adr/` has one. Edit the AGENTS.md, never the shim.
...
- Every `CLAUDE.md` (root or area) is a one-line `@AGENTS.md` shim; never put content in it. A PostToolUse hook in `.claude/settings.json` warns you when you edit one.
