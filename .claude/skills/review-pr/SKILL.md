...
**Timing matters in this repo.** A PR merges itself as soon as the required `checks` check passes, so a review of a ready PR often lands after the merge. Review a **draft** PR when the findings must arrive before the merge; on a ready PR, treat every finding as a follow-up commit or a follow-up issue.
...
Set `IS_AUTHOR=true` when `PR_AUTHOR` equals `CURRENT_USER`. GitHub rejects `APPROVE` and `REQUEST_CHANGES` on your own PR (HTTP 422), so when `IS_AUTHOR` is true the verdict can only be `COMMENT`. This is the normal case in a solo repo.
...
Record: which areas the PR touches (`src/savedState/`, `src/controlPanel/`, `src/settings/`, `options/`, `test/`, `adr/`, `.github/`, `manifest.json`), the total scope (additions/deletions, file count), the stated intent (commits + PR description), and any implementation proposal from the issue discussion.
...
1. **Pattern & architecture.** Does new code sit and register where similar code does? Find two or three existing examples of each new kind of thing and compare. Read files beyond the diff to see how the codebase does it elsewhere. Check the two hard rules of this repo: no obfuscated class name in any selector, and no click on a control the code cannot name.
2. **Completeness & issue alignment.** Does the PR deliver what the linked issue asks? Does it match any approach agreed in the issue comments? Flag missing deliverables and unexpected scope. A setting must appear in all four places (`DEFAULT_SETTINGS`, `normalizeSettings`, `optionsPage.html`, both functions of `optionsPage.js`); a missed one is a silent bug.
3. **Quality, safety & security.** Explicit JSDoc types; error handling (no silently swallowed errors); no new host permission and no new network request, because the extension makes none today; nothing that writes a user's photo data anywhere; no dead code, no over-engineering, no unnecessary duplication. Check `manifest.json` for a widened `matches` or a new permission.
4. **Testing & gaps.** Is red-green TDD respected (do the new tests assert real behavior, not just run the code)? What edge cases are untested? Name 2-3 realistic untested inputs and what would happen for each. Flag any change to a file that `adr/0006-testing-strategy.md` exempts from unit tests, because nothing automatic covers it. Confirm the two protected tests still exist: the empty-toolbar test and the late-Save-button test.

Also spawn, as siblings (they cannot run inside the analysis agents):

- **validate** (always): run `npm run check` and report format, types, and tests as pass or fail, with failing output verbatim.
- **pattern-scout** (only when the diff adds a new module, setting, page adapter, or scan behavior): find how similar ones are built here and report the convention with real examples.
...
4. **Classify severity.**
   - `[Required]`: blocks merge. Bugs that produce wrong output, security holes, missing deliverables from the issue, broken tests, type errors. In this repo, also: anything that could mark an unsaved photo as saved, and any blind click.
...
- **Operational results**: what passed and failed from validate and `gh pr checks`. Split failures into infra/config problems, real logic failures (these are findings), and external problems.
- **Findings index**: a numbered list of every finding that survived phase 4, so the user sees the total before the questions begin. Example:
  ```
  1. [Required]   Probe can read a stale toolbar - src/savedState/savedStateProbe.js:110 - fix in this PR
  2. [Suggestion] Missing test for an empty album - test/albumSavedStateScanner.test.js - fix in this PR
  3. [Suggestion] Pre-existing: poll interval duplicated - follow-up issue
  ```
...
Also write `.reviews/<PR_NUMBER>-review.md` (create the folder first: `mkdir -p .reviews`). The folder is gitignored, so it never reaches a commit.
