...
2. **For bugs, verify the bug actually exists in the current code.** If the code looks correct, report: "I checked the code and this appears to already be handled in `<file>:<line>`. Are you sure you want to create this issue?" Stop if the user agrees it is not a bug.
   - One caution specific to this repo: a bug report about wrong badges is often **not** a code bug. Check whether the reporter's `saveLabels` list covers the button name their Google Photos shows, and whether the cached scan results are simply stale. Ask for the **Copy diagnostics** output before you accept the report as a code defect.
...
1. **Area labels** (select all that apply, based on the affected code paths):
   ...
   No area labels are defined yet; propose none, and rebuild this section as a code-path-to-label table when labels exist. Check with `gh label list` before you assume.
...
- Good (change): "Currently a scan reads every photo again on each run; this issue makes it skip the photos already in the cache."
- Good (new capability): "Add a keyboard shortcut that starts a scan, so managing a shared album never needs the mouse."
- Bad (after only): "Skip cached photos during a scan." The reader cannot tell what is wrong today.
- Bad (before only): "The scan re-reads photos it already knows." States the gap but not what this issue makes true.
...
3. **Acceptance Criteria must describe observable behaviors, not implementation details.** AC answers "what does the system do?", not "what code do I write?". Good: "the grid shows no badge on a photo the user has just saved by hand". Bad: "add `refreshBadge` to `savedBadgeRenderer`".
...
6. **Steps to Reproduce must add value.** If reproduction is trivially "open an album and look", omit Steps and describe the condition in Context.
...
When area labels apply, extend the `--label` value comma-separated (for example `--label "scanning,waiting-for-human-check"`); with none, keep it exactly as above. Never emit a leading or trailing comma.
