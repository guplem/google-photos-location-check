...
**Know this before you start:** a PR in this repo **merges itself** the moment the required `checks` check passes. There is no human review gate. So the tests are the review, and the red-green TDD protocol in `AGENTS.md` is what stands between a change and `main`.
...
3. **Spawn the implementation agent** (Agent tool, `isolation: "worktree"`), pasting the explore reports into its prompt:
   ...
   > You are implementing GitHub issue #<NUMBER> for Google Photos auto Save Check.
   >
   > ## Issue
   >
   > <full title and body>
   ...
   > 4. **Red-green TDD, as `AGENTS.md` mandates:** write the failing test that pins each behavior before the code that makes it pass. A bug fix starts with a test that reproduces the bug. This repo auto-merges a green PR with no human review, so an untested change ships untested.
   > 5. Implement, following the explore findings and the issue.
   > 6. If you added or changed a setting, follow the `add-setting` skill: a setting must exist in `DEFAULT_SETTINGS`, in `normalizeSettings`, in `options/optionsPage.html`, and in both functions of `options/optionsPage.js`. A missed place makes the setting reset itself with no error.
   > 7. Run `npm run check` yourself (you cannot spawn the validate agent from inside here) and fix every failure you introduced before finishing.
...
- [ ] `npm run check` passes (format, types, tests)
- [ ] New behavior is covered by a test that failed before the change
- [ ] Loaded unpacked in Chrome and confirmed the change on a real Google Photos album
...
**The PR starts merging as soon as CI turns green.** Open it only when you are ready for that. Open a **draft** PR instead when you want CI to run without the merge, because `auto-merge.yml` skips drafts.

5. **Run the review cycle** (Section 7) before moving to the next step.

## 7. Review cycle (uses the review-pr skill)

Use the **review-pr** skill in `--no-verdict` mode. That mode runs unattended (it never asks the user anything), posts a COMMENT-only review to the PR, and writes a local findings file `.reviews/<PR_NUMBER>-review.md` with a clear verdict: `APPROVED` or `CHANGES_REQUESTED`. Invoke it with the Skill tool: `skill: "review-pr", args: "<PR_NUMBER> --no-verdict"`.

Because a green PR merges itself, run the review on a **draft** PR when you want the findings before the merge. On a ready PR, treat the cycle as a follow-up: a finding you act on becomes a new commit or a new PR.
...
- **Never push to `main` directly.** All work goes through branches and PRs, and the ruleset blocks a direct push anyway.
...
- **Verification is mandatory.** The implementation agent runs `npm run check` before pushing, and the orchestrator confirms a clean state with the **validate** agent.
...
- **Say when a change needs eyes.** `adr/0006-testing-strategy.md` exempts the thin DOM adapters from unit tests. When a step touched one, say so in the PR summary and in your report: no automatic check covers it.
