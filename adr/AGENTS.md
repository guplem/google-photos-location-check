...
This directory holds the Architecture Decision Records for Google Photos Location Check.

## What is an ADR?

An ADR records one architectural decision or cross-cutting standard, with its context, the decision, and its consequences. It answers "how do we always do X here": where the has-a-location signal comes from, how the code finds elements in a page it does not own, what the extension stores where, when a photo is read, and which files carry tests.
...
`NNNN-short-descriptive-title.md`, sequential, never renumbered. Gaps are fine. Use `TEMPLATE.md` as the starting point: Context, Decision, Consequences.
...
- A new setting, a new batch size, or a timing default. Those are code and belong in `AGENTS.md` rules at most.
- Anything a reader can derive from the code or from `README.md`.

## Creating or updating an ADR

Use the **adr-checker** agent. It enforces the full rules (one decision per ADR, at most about 2 pages, alternatives with rejection rationale, update-in-place, source-of-truth-is-the-code) and writes the file. Index every ADR as a one-line row in the root `AGENTS.md`.
